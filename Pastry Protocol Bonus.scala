...
case class startPastry(num_of_nodes:Int,num_of_requests:Int,num_of_fail:Int,mainnode:ActorRef)
...
case class editleafSetSmaller(nodeId1:String,nodeId2:String)
case class editleafSetLarger(nodeId1:String,nodeId2:String)
...
class Node extends Actor {
  ...
  var numfin:Int=0
  var hops:Int=0
  var alivenodes:ArrayBuffer[String]=ArrayBuffer[String]()
  ...
  def receive = {
    ...
    case failnode() => {
      if(leafSetLarger.length!=0)
      {
      for ( i <- 0 until leafSetSmaller.length)
        map.apply(leafSetSmaller(i)) ! editleafSetSmaller(nodeId,leafSetLarger(0))
      }
      else
      {
      for ( i <- 0 until leafSetSmaller.length)
        map.apply(leafSetSmaller(i)) ! editleafSetSmaller(nodeId,"-1")
      } 
      if(leafSetSmaller.length!=0)
      {
      for( i <- 0 until leafSetLarger.length)
        map.apply(leafSetLarger(i)) ! editleafSetLarger(nodeId,leafSetSmaller(leafSetSmaller.length-1))
      }
      else
      {
      for( i <- 0 until leafSetLarger.length)
        map.apply(leafSetLarger(i)) ! editleafSetLarger(nodeId,"-1")
      }
      context.stop(self)
    }
    case editleafSetSmaller(nodeId1,nodeId2) => {
      for( i <- 0 until leafSetLarger.length)
        if(leafSetLarger(i).equals(nodeId1) && !nodeId2.equals("-1"))
          leafSetLarger(i)=nodeId2
        else if(nodeId2.equals("-1"))
          leafSetLarger.remove(i)
    }
    case editleafSetLarger(nodeId1,nodeId2) => {
      for( i <- 0 until leafSetSmaller.length)
        if(leafSetSmaller(i).equals(nodeId1) && !nodeId2.equals("-1"))
          leafSetSmaller(i)=nodeId2
        else if(nodeId2.equals("-1"))
          leafSetSmaller.remove(i)      
    }
  }
}
class mainNode extends Actor {
  ...
  def buildTopology(num_of_nodes : Int,num_of_requests :Int,mainnode:ActorRef)  = {
    ...
    for(i <- 0 until num_of_nodes)
      node(i) ! sendrequest()
    for(i <- 0 until num_of_fail)
    {
      var rand=Random.nextInt(num_of_nodes)
      node(rand) ! failnode()
    }
      
  }
  def receive = {
    case startPastry(num_of_nodes,num_of_requests,no_of_fail,mainnode) => {
      var i:Int = 0
      num_of_fail=no_of_fail
    ...
    case destinationReached(hops) => {
      ...
      if(dest==((num_of_nodes*num_of_requests)-(num_of_fail*num_of_requests)))
      {
...
object project3bonus extends App {
  ...
  var no_of_fail=args(2).toInt
  val mainnode = system.actorOf(Props[mainNode],name="mainnode")
  mainnode ! startPastry(no_of_nodes,no_of_requests,no_of_fail,mainnode)
}